...
function liveAutomationControlPanel() {
  ...
  const dryRun = !cfg || cfg.dryRunMode !== false;
  ...
  const status = emergency
    ...
    : automationOn
    ? { text: dryRun ? "RUNNING — DRY RUN" : "RUNNING — LIVE", cls: dryRun ? "ac-dryrun" : "ac-live" }
  ...
  <section class="panel ac-panel">
    ...
    ${dryRun ? `<div class="ac-dryrun-banner">🧪 DRY RUN MODE — transaction intents are generated and logged, but NOTHING is submitted on-chain.</div>` : `<div class="ac-live-banner">⚠ LIVE MODE — real transactions will be submitted. Confirm you intend this.</div>`}
    ...
    <div class="ac-grid">
      ...
      ${row("Mode", dryRun ? "DRY RUN" : "LIVE", dryRun ? "ac-text-dryrun" : "ac-text-live")}
