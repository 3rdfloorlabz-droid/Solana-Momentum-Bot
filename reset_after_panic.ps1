...
try {
  ...
  # A3 config change audit (append-only; best-effort; never blocks reset).
  try {
    $auditChangeId = [guid]::NewGuid().ToString()
    $auditMode = [string]$config.executionMode
    $auditLiveArmed = Get-ConfigLiveArmed -Cfg $config
    Append-ConfigAudit -Field "emergencyStop" -OldValue $oldEmergencyStop -NewValue $false -RiskLevel "CRITICAL" -Reason "manual reset_after_panic.ps1" -Source "reset_after_panic.ps1" -ChangeId $auditChangeId -ModeAtChange $auditMode -LiveArmedAtChange $auditLiveArmed
    Append-ConfigAudit -Field "automationEnabled" -OldValue $oldAutomationEnabled -NewValue $true -RiskLevel "CRITICAL" -Reason "manual reset_after_panic.ps1" -Source "reset_after_panic.ps1" -ChangeId $auditChangeId -ModeAtChange $auditMode -LiveArmedAtChange $auditLiveArmed
