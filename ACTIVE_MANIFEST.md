...
A2i/A2j dashboard auth guard (`test_dashboard_auth_guards.js`) is **active in `run_safety_tests.js` (14/14)** for static checks (POST route inventory, forbidden recovery routes/primitives, A2c preview boundary, **A2j fail-closed auth wrapper** on `/control/start`, `/control/stop`, `/control/emergency`). **A2k** (`test_dashboard_auth_behavior.js`) adds isolated HTTP behavioral auth tests using temp fixtures via `TRACKTA_RUNTIME_ROOT`. **A2m** (`recovery_audit.js` + `test_recovery_audit.js`) implements the append-only recovery audit writer (temp-fixture tests only); **not wired to recovery execution**. **A2p** (`test_recovery_route_guards.js`) protects future recovery route boundaries — **recovery POST routes remain unimplemented**. **A2q** (`fake_recovery_harness.js` + `test_fake_recovery_harness.js`) provides a deterministic fake process/runtime model for tests — **does not touch real TracktaOS processes**. **A2r** (`fake_recovery_flow.js` + `test_low_risk_recovery_behavior.js`) proves the future low-risk human-confirmed recovery lifecycle using fake harness and temp audit ledger only — **no real recovery routes or process execution exist**.
