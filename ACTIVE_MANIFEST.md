...
| `r28_manual_quote_observation_decision_check.js` | R28 manual quote observation decision session check (`analysis/r28_manual_quote_observation_decision_status.json`) |
| `r29_real_quote_observer.js` | R29 gated real quote observer — default DISABLED; `--observe-once` only (`analysis/r29_real_quote_observation_status.json`) |
| `r30_quote_observation_results_review.js` | R30 real quote observation results review (`analysis/r30_quote_observation_results_review.json`) |
| `r31_r32_quote_observation_hardening_check.js` | R31-R32 hardening + batch plan check (`analysis/r31_r32_quote_observation_hardening_status.json`) |
| `r33_clean_quote_observation_review.js` | R33 clean quote observation review + schema v2 validation (`analysis/r33_clean_quote_observation_review.json`) |
| `r34_manual_quote_batch_review.js` | R34 small manual quote batch review (`analysis/r34_manual_quote_batch_review.json`) |
| `r35_quote_batch_shadow_readiness.js` | R35 quote batch shadow readiness review (`analysis/r35_quote_batch_shadow_readiness.json`) |
| `r36_shadow_execution_harness.js` | R36 shadow execution harness — simulation only (`analysis/shadow_execution_decisions.jsonl`, `analysis/r36_shadow_execution_status.json`) |
| `r37_shadow_results_wallet_readiness.js` | R37 shadow results + wallet setup readiness (`analysis/r37_shadow_results_wallet_readiness.json`) |
| `r38_research_wallet_secret_design_check.js` | R38 research wallet + secret storage design (`analysis/r38_research_wallet_secret_design_status.json`) |
| `micro_live_preflight.js` | R8A micro-live engineering proof preflight (`analysis/micro_live_preflight.json`) |
| `micro_live_caps.js` | Track A operator caps schema/loader (read-only) |
| `micro_live_guardrails.js` | Track A hard micro-live guardrail checker (`analysis/micro_live_guardrails_check.json`) |
| `secret_safety_scan.js` | R39 read-only secret-risk pattern scanner (`analysis/secret_safety_scan.json`) |
| `mock_signer.js` | R40 mock signer harness — fake signing only (not integrated with executor) |
| `signer_plan_preflight.js` | R41 local signer plan preflight (`analysis/signer_plan_preflight.json`) |
| `local_signer.js` | R41B local signer safety stub — no real signing |
| `signer_provider.js` | R41B signer provider selection (mock/local_stub only) |
| `micro_live_rpc_preflight.js` | R41C dedicated RPC + signer readiness (`analysis/r41c_rpc_signer_readiness.json`) |
| `micro_live_rpc_config.js` | R41D dedicated RPC operator setup loader (`analysis/r41d_rpc_operator_setup.json`) |
| `r43a_pre_approval_readiness.js` | R43A final pre-approval readiness review (`analysis/r43a_pre_approval_readiness.json`) |
| `r43b_operator_caps_approval_check.js` | R43B operator caps approval check (`analysis/r43b_operator_caps_approval_check.json`) |
| `r43c_local_signer_readiness.js` | R43C guarded real local signer readiness (`analysis/r43c_local_signer_readiness.json`) |
...
| **Track A Micro-Live Hard Guardrails** | **BUILT — GUARDRAIL INFRASTRUCTURE ONLY** (2026-06-23) — [docs/TRACK_A_MICRO_LIVE_GUARDRAILS.md](./docs/TRACK_A_MICRO_LIVE_GUARDRAILS.md) · caps loader + guardrail checker · **live trading NOT approved** |
| **R39 Signer Safety Design** | **DEFINED — SIGNER DESIGN ONLY — NOT READY FOR IMPLEMENTATION** (2026-06-23) — [docs/R39_SIGNER_SAFETY_DESIGN.md](./docs/R39_SIGNER_SAFETY_DESIGN.md) · secret boundary review · **no key handled** |
...
| **R41B Local Signer Safety Stubs** | **BUILT — STUB ONLY — REAL SIGNING NOT IMPLEMENTED** (2026-06-23) — [docs/R41B_LOCAL_SIGNER_SAFETY_STUBS.md](./docs/R41B_LOCAL_SIGNER_SAFETY_STUBS.md) · **no executor integration** |
| **R41C Dedicated RPC + Signer Readiness** | **BUILT — READINESS CHECK ONLY — NOT APPROVED** (2026-06-23) — [docs/R41C_DEDICATED_RPC_AND_SIGNER_READINESS.md](./docs/R41C_DEDICATED_RPC_AND_SIGNER_READINESS.md) · dedicated RPC + stub signer readiness · **live trading NOT approved** |
| **R41D Dedicated RPC Operator Setup** | **BUILT — LOCAL CONFIG ONLY — NOT APPROVED** (2026-06-23) — [docs/R41D_DEDICATED_RPC_OPERATOR_SETUP.md](./docs/R41D_DEDICATED_RPC_OPERATOR_SETUP.md) · `TRACKTA_MICRO_LIVE_RPC_URL` / gitignored local file · **no real RPC committed** |
| **R43A Final Pre-Approval Readiness Review** | **BUILT — PRE-APPROVAL ONLY — NOT LIVE EXECUTION** (2026-06-23) — [docs/R43A_FINAL_PRE_APPROVAL_READINESS_REVIEW.md](./docs/R43A_FINAL_PRE_APPROVAL_READINESS_REVIEW.md) · **live trading NOT approved** |
| **R43B Operator Caps Approval Record** | **RECORDED — ENGINEERING PROOF ONLY — NOT LIVE TRADING** (2026-06-28) — [docs/R43B_OPERATOR_CAPS_APPROVAL_RECORD.md](./docs/R43B_OPERATOR_CAPS_APPROVAL_RECORD.md) · **full live trading NOT approved** |
| **R43C Real Local Signer Under Guardrails** | **BUILT — GUARDED — NOT LIVE TRADING — NO SUBMISSION** (2026-06-23) — [docs/R43C_REAL_LOCAL_SIGNER_GUARDRAILS.md](./docs/R43C_REAL_LOCAL_SIGNER_GUARDRAILS.md) · **no executor integration** |
| **R43D Final Proof Preflight** | **BUILT — READ-ONLY PREFLIGHT — NOT TRANSACTION** (2026-06-23) — [docs/R43D_FINAL_PROOF_PREFLIGHT.md](./docs/R43D_FINAL_PROOF_PREFLIGHT.md) · **proof readiness only; live trading NOT approved** |
...
| **Track B B1 Thesis** | **DEFINED — TRACK A CLOSED — RESEARCH ONLY** (2026-06-29) — [docs/TRACK_B_B1_THESIS.md](./docs/TRACK_B_B1_THESIS.md) · **closes Track A engineering proof; opens Track B research; live trading NOT approved** |
| **Track B B2 Data Collection Plan** | **DEFINED — OBSERVATION ONLY — NOT LIVE APPROVAL** (2026-06-29) — [docs/TRACK_B_B2_DATA_COLLECTION_PLAN.md](./docs/TRACK_B_B2_DATA_COLLECTION_PLAN.md) · **metric ownership, cadence, segmentation, shadow design; live trading NOT approved** |
| **B2A 24-Hour Observation Run Plan** | **DEFINED — OBSERVATION ONLY — NOT LIVE APPROVAL** (2026-06-29) — [docs/B2A_24H_OBSERVATION_RUN_PLAN.md](./docs/B2A_24H_OBSERVATION_RUN_PLAN.md) · **24h scanner/monitor/dry-run collection runbook; no live trades** |
...
| **R33 Clean Quote Observation Review** | **COMPLETE — CLEAN QUOTE OBSERVATION REVIEWED — TRADING STILL BLOCKED** (2026-06-23) — [docs/R33_CLEAN_QUOTE_OBSERVATION_REVIEW.md](./docs/R33_CLEAN_QUOTE_OBSERVATION_REVIEW.md) |
| **R34 Small Manual Quote Batch Review** | **DEFINED — SMALL MANUAL QUOTE BATCH REVIEW DEFINED — OBSERVATION ONLY** (2026-06-23) — [docs/R34_SMALL_MANUAL_QUOTE_OBSERVATION_BATCH_REVIEW.md](./docs/R34_SMALL_MANUAL_QUOTE_OBSERVATION_BATCH_REVIEW.md) |
| **R35 Quote Batch Results + Shadow Readiness** | **COMPLETE — QUOTE BATCH REVIEWED — READY FOR SHADOW EXECUTION HARNESS DESIGN** (2026-06-23) — [docs/R35_QUOTE_BATCH_RESULTS_AND_SHADOW_READINESS.md](./docs/R35_QUOTE_BATCH_RESULTS_AND_SHADOW_READINESS.md) |
...
| `examples/r28_manual_quote_observation_decision.example.json` | R28 example-only operator decision record (default NOT_DECIDED / HOLD) |
| `analysis/r28_manual_quote_observation_decision_status.json` | R28 manual decision session status (gitignored) |
| `operator_records/r29_quote_observation_approval.json` | R29 operator approval record (observation-only; no secrets) |
| `examples/r29_real_quote_observation_config.example.json` | R29 example config (`active: false`) |
| `analysis/r29_real_quote_observation_status.json` | R29 observer status (gitignored) |
| `analysis/r30_quote_observation_results_review.json` | R30 observation results review (gitignored) |
| `examples/r31_real_quote_candidates.example.json` | R31 real-mint manual observation candidates (default 100 bps) |
| `analysis/r31_r32_quote_observation_hardening_status.json` | R31-R32 hardening + batch plan status (gitignored) |
| `analysis/r33_clean_quote_observation_review.json` | R33 clean observation review + schema v2 validation (gitignored) |
| `analysis/r34_manual_quote_batch_review.json` | R34 manual quote batch review (gitignored) |
| `analysis/r35_quote_batch_shadow_readiness.json` | R35 quote batch shadow readiness review (gitignored) |
| `analysis/shadow_execution_decisions.jsonl` | R36 simulated shadow decisions (gitignored) |
| `analysis/r36_shadow_execution_status.json` | R36 shadow harness status (gitignored) |
| `analysis/r37_shadow_results_wallet_readiness.json` | R37 shadow results + wallet setup readiness (gitignored) |
| `analysis/r38_research_wallet_secret_design_status.json` | R38 research wallet secret design status (gitignored) |
| `analysis/micro_live_preflight.json` | R8A micro-live preflight status (gitignored) |
| `analysis/micro_live_guardrails_check.json` | Track A micro-live guardrail check status (gitignored) |
| `analysis/secret_safety_scan.json` | R39 secret safety scan report (gitignored) |
...
| `analysis/r41c_rpc_signer_readiness.json` | R41C dedicated RPC + signer readiness status (gitignored) |
| `analysis/r41d_rpc_operator_setup.json` | R41D dedicated RPC operator setup status (gitignored) |
| `analysis/r43a_pre_approval_readiness.json` | R43A final pre-approval readiness review status (gitignored) |
| `analysis/r43b_operator_caps_approval_check.json` | R43B operator caps approval check status (gitignored) |
...
| `analysis/b2a_24h_observation_status.json` | B2A 24h observation status snapshots (gitignored) |
| `soak_runs/b2a_24h_observation_start.json` | B2A observation run start marker (gitignored) |
