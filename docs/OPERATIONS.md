...
- **A2s low-risk recovery routes** (`POST /recovery/plan/:actionId`, `POST /recovery/confirm/:actionId`) require **`DASHBOARD_CONTROL_TOKEN`** and header **`X-Trackta-Control-Token`**. Browser forms do **not** trigger recovery. Allowed actions only: `restart-scanner`, `restart-paper-monitor`, `restart-wallet-monitor`, `restart-dashboard`. **A2s uses simulated execution** — no real process spawn/kill yet. Live trading remains prohibited. Do not manually edit `recovery_actions.jsonl`.
