...
- **A2r behavioral recovery tests** (`fake_recovery_flow.js`, `test_low_risk_recovery_behavior.js`) simulate the future human-confirmed low-risk recovery lifecycle with fake harness and temp `recovery_actions.jsonl` only. **These tests do not give operators recovery commands** and do not authorize real process restarts.
