...
| **A2c preview-only UI** | Dashboard **Recovery Action Preview** exists (`recoveryActionPreviewSection()`) — command text and eligibility only; **does not execute recovery**. Future execution-capable UI requires authentication, `recovery_actions.jsonl` audit logging, stronger validation, and explicit approval. Do not treat preview eligibility as permission to act. |
