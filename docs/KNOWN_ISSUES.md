...
| **Status** | **`paper_trades.json` dual-writer partially resolved** (2026-06-22, Sprint 4 A1a). Ownership split via `paper_positions_store.js`: `paper_trades.json` is now **scanner-owned, append-only** (monitor no longer rewrites it); the monitor owns a separate mutable lifecycle store **`paper_positions.json`** (single writer, atomic temp-rename), keyed by `entryId = timestamp_address_pairAddress`, idempotently seeded from the existing ledger with no history loss. Scanner `alreadyOpen`/`recentlyLost` and the dashboard read a merged view (ledger ⊕ store) with ledger fallback when the store is absent. **Remaining (planned):** `live_config.json` atomic writes (A1b) and `live_positions.json`/`observation_dedup.json` single-writer atomicity remain open; concurrent multi-process writers elsewhere unchanged. |
