...
| Field | Detail |
|-------|--------|
| **Description** | `dashboard_server.js` on port 3000 toggles `automationEnabled` and related config via three config-control POST routes (`/control/start`, `/control/stop`, `/control/emergency`). **A2j (2026-06-23):** those routes are now wrapped by a fail-closed auth guard — `DASHBOARD_CONTROL_TOKEN` env var plus `X-Trackta-Control-Token` request header. Missing/empty env, missing header, wrong token, or query-string token all deny with a generic message; denied requests do not mutate config or write audit/control events. **A2j does not add recovery execution.** Read-only GET routes remain unauthenticated. Plain HTML START/STOP/EMERGENCY forms do not send the header and will fail until the operator uses a header-capable client (see [OPERATIONS.md](./OPERATIONS.md) → Dashboard Control Auth). Control-surface inventory: [A2G_DASHBOARD_AUTH_READINESS_REVIEW.md](./A2G_DASHBOARD_AUTH_READINESS_REVIEW.md). Recovery-audit architecture (planning only): [A2F_AUTH_RECOVERY_AUDIT_PLAN.md](./A2F_AUTH_RECOVERY_AUDIT_PLAN.md). Auth guard tests: [A2H_AUTH_GUARD_TEST_DESIGN.md](./A2H_AUTH_GUARD_TEST_DESIGN.md). **`test_dashboard_auth_guards.js`** is active in `run_safety_tests.js` (9/9) for static checks including A2j auth wrapper presence. Recovery execution remains blocked pending recovery audit, stronger validation, and explicit approval. |
...
| **A2d soak (Sprint 4)** | **Accelerated validation complete — PASS WITH RISK ACCEPTED** (2026-06-23). Full **72-hour soak was not completed**; operator accepted abbreviated evidence (~1h, CP0–CP4). Review: [A2D_SOAK_REVIEW.md](./A2D_SOAK_REVIEW.md). Plan: [A2D_SOAK_VALIDATION_PLAN.md](./A2D_SOAK_VALIDATION_PLAN.md). Runbook: [OPERATIONS.md](./OPERATIONS.md) → **A2d Soak Operation**. Checkpoints: [A2D_SOAK_CHECKPOINT_LOG.md](./A2D_SOAK_CHECKPOINT_LOG.md). **A2d does not authorize recovery execution, buttons, or live trading.** **Any future recovery execution requires stronger validation** (full soak or equivalent) plus explicit human authorization before A2c execution — preview-only UI is the only authorized next step. |
...
| **Duplicate process sets** | Two concurrent scanner/monitor/executor/wallet windows (e.g. old + new `start_fomo.ps1`) muddy soak evidence and can reintroduce race risk. Confirm a single canonical set before continuing checkpoints; record cleanup in the log. |
...
| **A2c preview-only UI** | Dashboard **Recovery Action Preview** exists (`recoveryActionPreviewSection()`) — command text and eligibility only; **does not execute recovery**. Intentionally non-executing and now **regression-guarded** by `test_recovery_preview_guards.js` (part of `node run_safety_tests.js`, 8/8): fails if the preview gains buttons/forms/POST routes/`spawn`/`exec`/`child_process`/`process.kill`/`recovery_actions.jsonl` writes, or if `app.post` routes drift from the three known config-control routes. Milestone complete — see [A2C_REVIEW.md](./A2C_REVIEW.md). Execution-capable recovery UI remains **blocked** pending authentication, `recovery_actions.jsonl` audit logging, stronger validation, and explicit approval — these prerequisites are now **designed** (planning only) in [A2F_AUTH_RECOVERY_AUDIT_PLAN.md](./A2F_AUTH_RECOVERY_AUDIT_PLAN.md) (auth model, recovery-audit schema, prechecks/postchecks, fail-closed rules, guardrails; `recovery_actions.jsonl` not created). The same auth design must retrofit the three pre-existing unauthenticated config-control routes. Do not treat preview eligibility as permission to act. |
