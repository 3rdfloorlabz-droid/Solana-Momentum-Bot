...
| **R7b strategy data collection (Sprint 4)** | **PLAN COMPLETE — collection IN PROGRESS.** Defines fresh-sample thresholds (≥30 closed paper trades, ≥7 active-market days, thesis/exit mix minimums, PF ≥1.20 fresh) before **R8 Risk Controls Review** may be considered. Daily read-only progress: `node r7b_daily_summary.js` → `analysis/r7b_daily_summary.json`. Plan: [R7B_STRATEGY_DATA_COLLECTION_PLAN.md](./R7B_STRATEGY_DATA_COLLECTION_PLAN.md). **Does not approve live trading or loosen filters.** |
