...
| Field | Detail |
|-------|--------|
| **Description** | `dashboard_server.js` on port 3000 toggles `automationEnabled` and related config without auth. The three pre-existing config-control POST routes (`/control/start`, `/control/stop`, `/control/emergency`) remain **unauthenticated-localhost mutation surfaces** — A3-audited but not gated. The control-surface inventory and phased remediation path (auth → audit → tests → execution) are documented in [A2G_DASHBOARD_AUTH_READINESS_REVIEW.md](./A2G_DASHBOARD_AUTH_READINESS_REVIEW.md) (review only); the auth/audit architecture is designed in [A2F_AUTH_RECOVERY_AUDIT_PLAN.md](./A2F_AUTH_RECOVERY_AUDIT_PLAN.md). **No recovery execution should be added before auth + audit + tests exist.** Auth guard tests are designed in [A2H_AUTH_GUARD_TEST_DESIGN.md](./A2H_AUTH_GUARD_TEST_DESIGN.md). **A2i (tests-first):** `test_dashboard_auth_guards.js` is active in `run_safety_tests.js` (9/9) for **static checks only** (route inventory, forbidden recovery routes/primitives, A2c preview boundary). **Mutation routes remain unauthenticated until A2j** implements the auth wrapper; pending auth-enforcement checks run via `node test_dashboard_auth_guards.js --pending-a2j` (expected to fail until A2j). A2j will implement auth and activate denial behavior tests. |
