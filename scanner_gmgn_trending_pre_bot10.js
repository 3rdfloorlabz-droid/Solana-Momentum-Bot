...
function recentlyTraded(address, hours = 24) {
  try {
...
function recentlyTraded(address, hours = 12) {
  try {
    const cutoff = Date.now() - (hours * 60 * 60 * 1000);

    const trades = loadPaperTrades(); // use whatever function you already use
...
async function scan() {
  ...
  for (const c of passedDex) {
    ...
    c.bundlerRate = Number(c.trend.bundler_rate || 0);
